...
class Drawer:
    ...
    def draw_update(self) -> None:
        ...
        self.draw_board(self.board, (228, 25), (760, 569))
        pygame.display.flip()
        self.clock.tick(60)

    def draw_board(self, board: board.Board, start_coord: tuple, end_coord: tuple) -> None:
        """Draws stones from board's world list.\n
        Requires start and end coord of in-game display board"""
        current_coord = list(start_coord[:])
        board_ptr = board.get_board() #Create a reference to the world board
        MAGIC_NUMBER = 1.25

        size = len(board_ptr)
        space_size = (end_coord[0] - start_coord[0]) // size # Get distance between the intersections  
        for pos_y, y_dim in enumerate(board_ptr):
            for pos_x, x_dim in enumerate(y_dim):
                color = ""
                stone_color = board_ptr[pos_y][pos_x].getColor()
                current_coord = [start_coord[0] + pos_x * space_size *MAGIC_NUMBER, start_coord[1] + pos_y * space_size *MAGIC_NUMBER]
